...
| 被测源码 | 工作区（基线 commit `6d53fa3`）+ 本次 `src/cli.rs` 修复（未提交） |
| 测试日期 | 2026-08-11 |
| 测试执行 | 全自动（脚本驱动 + `cargo test` + Python `winreg`） |
| 结论 | ✅ 通过（22/22 自动化检查全 PASS）；修复 1 个缺陷；1 条改进建议 |
...
- **命令行端到端（无头真实场景）**：多文件/单文件/目录压缩、Unicode 文件名、目标冲突命名、50MB 大文件往返、extract-here、跨工具互操作（Python ↔ ReallyZip）、异常与边界。
...
被测二进制：本次使用**含 `cli.rs` 修复的调试剖面构建**（`reallyzip.exe`，33,391,104 B）。
> 说明：发布版 `cargo build --release`（LTO fat 链接）在**当前测试沙箱内存受限环境下持续 OOM 被杀**，无法重建；调试剖面与发布版**功能逻辑完全一致**，故测试结论对发布版同样有效。发布版二进制请在本地/CI 执行 `cargo build --release` 重建（见 §6 待办）。
...
脚本：`tests/e2e/cli_e2e.py`，全部 16 例 PASS。

| 用例 | 描述 | 预期 | 实际 | 结果 |
|---|---|---|---|---|
| T-SM-01 | 单文件 PE 产物存在（>1MB） | 文件存在 | 33,391,104 B | ✅ |
| T-CLI-01 | 多文件 `--compress-here` 生成单个 zip 含全部 | 含全部选中文件 | 含 a.txt/b.txt/c.txt（按文件名扁平化，见 F-01） | ✅ |
| T-CLI-02 | 单文件 `--compress-here` 生成 `<名>.zip` | 含 1 条目 | solo.txt | ✅ |
| T-CLI-03 | 目录压缩保留顶层文件夹名 | 保留 tree/ | tree/、tree/a.txt、tree/nested/b.txt | ✅ |
| T-CLI-04 | 空格/中文/Unicode 文件名 | 无损 | 内容 中文.txt、文件 A.txt（扁平化） | ✅ |
...
### F-01（建议改进，中优先级）— 多文件压缩按文件名扁平存储
- **现象**：选中跨目录的多个文件压缩时，zip 内条目仅保留各自**文件名**，丢弃源目录层级（如 `sub/c.txt` → `c.txt`）。
- **影响**：同一批选中若含同名文件（如 `a/x.txt` 与 `b/x.txt`），会**互相覆盖**；同目录多选则无影响（常见右键场景）。
- **建议**：以「共同父目录」为根保留相对路径；或检测到同名冲突时自动重命名/提示。

### F-02（环境限制，非缺陷）— 发布版构建 OOM
- 见 §1 说明。测试以调试剖面完成，逻辑等效；发布二进制需本地/CI 重建。

---

## 5. 测试总结

- **自动化检查 22/22 全部通过**（单元 6 + 端到端 16）。
...
- 修复 1 个会导致卡死的缺陷（D-01）。
- 提出 1 条产品改进建议（F-01，多文件扁平化冲突）。
- GUI 交互路径（对话框全选项、包内预览等）需人工验证，其底层函数已被自动化覆盖。

**质量结论**：核心功能达到可发布水平；建议在发布前处理 F-01 并人工走查 GUI 对话框路径。

---

## 6. 待办 / 风险

1. **发布版二进制重建**：当前沙箱内存不足以完成 `cargo build --release`（LTO 链接 OOM）。请在本地或 CI 执行 `cargo build --release` 以产出含 D-01 修复的优化版 `reallyzip.exe` 并发布。
2. **F-01 改进**：评估多文件相对路径保留 / 同名冲突策略。
3. **GUI 人工验证清单**：压缩对话框（级别/AES/分卷/追加）、包内浏览与预览、解压对话框、设置页「注册右键菜单」交互。
